...
def create_model(vocabulary_size, embedding_size, T, L, D):
    ...
    encoder = LSTM(embedding_size, return_sequences=True, return_state=True)
    attented_encoder = ExternalAttentionRNNWrapper(encoder)

    output = TimeDistributed(Dense(vocabulary_size, activation="softmax"), name="output")

    # for training purpose
    attented_encoder_training_data, _, _ = attented_encoder([captions, image_features], initial_state=[initial_state_h, initial_state_c])
    ...
    attented_encoder_inference_data, inference_encoder_state_h, inference_encoder_state_c = attented_encoder(
    ...
    inference_model = Model(
        ...
        outputs=[inference_output_data, inference_encoder_state_h, inference_encoder_state_c]
    )
